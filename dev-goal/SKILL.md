...
| **A: Action** | `code-impl` + `test-suite` | 传递选定方案 → 监督执行（code-impl 内含 commit 生成） → 失败恢复 |
...
Agent: [A-Phase: 委托 code-impl + test-suite]
       ├── Skill(code-impl) → 读 front-review.md + plan.md → 编码 → 自动生成 commit → 写 code-changes.md
...
| O: devplan | 1-2 种思路简要对比 | ≥2 方案定性对比 | ≥3 方案 + 接口契约 + 依赖图 |
| A: code-impl | 快速实现，≤3 commit | 标准实现 + 回滚点 + 子目标 commit | 每子目标独立回滚点 + commit |
| A: test-suite | lint + test -cover | + race -count=3 + benchmark | + fuzz + 并发压力 + 泄漏检测 |
| L: finish-review | 快速审查 | 完整五轴审查 | 同标准 + 设计模式总结 |
| 输出 | 仅 memory（有价值时） | 1 个工作目录（5 份输出） | 1 个工作目录（含接口契约 + 依赖图） |

> **Python 项目**：Go 命令自动映射为 Python 等价命令（`go vet`→`ruff`/`mypy`，`go test -race`→`asyncio.gather` 竞态测试 等），详见 `code-impl` 和 `test-suite` 的语言附录。
...
请按选定方案执行编码，按子目标 1:1 增量提交。
编码完成自动生成 commit message（展示给我确认）。
将编码结果写入 工作目录/code-changes.md。
```

`code-impl` 完成后，dev-goal 追加摘要（含 commit 记录）：
...
dev-goal (调度层)
  ...
  │
  ├─ A: Skill(code-impl) → code-changes.md（含 commit 记录）
